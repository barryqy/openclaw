...
if helper_start in text:
    ...
    text = text[:start] + helper_insert + text[end:]
else:
    if anchor not in text:
        raise SystemExit(f"Expected helper anchor not found in {path}")
    text = text.replace(anchor, helper_insert + anchor, 1)

request_patch = "            if (shouldForceNonStreamingOpenAICompletions(model)) {\n                await emitNonStreamingOpenAICompletion(client, model, params, options, output, stream);\n                return;\n            }\n"
if request_patch not in text:
    payload_anchor = re.compile(r"(if \(nextParams !== undefined\) {\n\s*params = nextParams;\n\s*}\n)")
    text, replaced = payload_anchor.subn(r"\1" + request_patch, text, count=1)
    if replaced != 1:
        raise SystemExit(f"Expected payload normalization anchor not found in {path}")
...
if ! command -v openclaw >/dev/null 2>&1; then
  ...
  SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g --prefix "${HOME}/.local" "openclaw@${OPENCLAW_NPM_VERSION}"
  export PATH="${HOME}/.local/bin:${PATH}"
  hash -r
fi
